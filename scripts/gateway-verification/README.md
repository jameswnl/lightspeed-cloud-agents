...
| `verify_l7_rules_narrowing_fix.py` | The #247 fix — narrowing the bundled `ProviderProfile` endpoint AND the sandbox-owned `llm_provider` endpoint (both needed: OpenShell's policy merge unions a bare `access` preset with the other side's `rules` instead of letting the narrower one win) actually restricts real gateway traffic — an allowlisted call succeeds and a non-allowlisted one is blocked with a real 403, not just that `CreateSandbox` succeeds. |
