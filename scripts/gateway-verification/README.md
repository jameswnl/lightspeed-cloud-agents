...
| `verify_provider_profile_fix.py` | The #244 fix — `_ensure_provider_profile()` registers a real, idempotent `ProviderProfile` for "openai" (OpenShell ships no builtin one), and a spawn with `LIGHTSPEED_PROVIDER=openai` succeeds once it's registered. |
