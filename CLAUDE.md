...
| Orphan reconciliation | `temporal_entrypoint.py` | `temporal/test_startup_reconciliation.py` -- **known gap ([issue #224](https://github.com/jameswnl/lightspeed-cloud-agents/issues/224))**: this test only exercises the call pattern against a mocked spawner. `OpenShellSpawner._do_list_active()` returns names from an in-memory dict (`self._sandbox_names`) that resets on process restart, so real cross-restart orphan recovery does not currently work for OpenShellSpawner -- unlike the deleted `KubernetesSpawner`/`PodmanSpawner`, which queried real infra-level labels. |
