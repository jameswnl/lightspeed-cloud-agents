...
| Credential injection (Provider API) | `openshell_spawner.py` | `spawner/test_openshell_spawner.py` -- OpenShell ships no builtin `ProviderProfile` for "openai"/"anthropic" (only aws/aws-bedrock/aws-s3/claude-code/codex/copilot/cursor/deepinfra/github/google-cloud/google-vertex-ai/nvidia/pypi are builtins), so a `Provider` created with one of those two types would otherwise silently get neither its credential injected nor network-egress policy applied (gateway logs `"provider type has no profile"`) even though `CreateProvider` itself reports success (issue #244). `OpenShellSpawner._ensure_provider_profile()` idempotently imports a bundled default (`spawner/provider_profiles.py`) scoped to the spawner's own workspace before `_create_provider()` runs. |
