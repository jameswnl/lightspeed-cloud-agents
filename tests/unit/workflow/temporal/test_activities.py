...
class TestRunSandboxStep:
    ...
    async def test_context_includes_prior_steps(self, mocker: MockerFixture) -> None:
        ...
        mock_build_ctx = mocker.patch(
            "cloud_agents.workflow.executor.temporal.activities.build_sandbox_context",
            return_value={},
        )
...
class TestTLSWiring:
    ...
    async def test_tls_error_emits_audit_and_metric(self, mocker: MockerFixture) -> None:
        ...
        mock_emit = mocker.patch("cloud_agents.workflow.executor.temporal.activities.emit_audit")
        mock_metric = mocker.patch(
            "cloud_agents.workflow.executor.temporal.activities.ls_sandbox_tls_errors_total"
        )
...
class TestNotificationActivity:
    ...
    async def test_notification_sends_with_correlation_id(
        ...
        from cloud_agents.workflow.executor.temporal.activities import send_approval_notification

        mock_notifier_cls = mocker.patch(
            "cloud_agents.workflow.executor.temporal.activities.NullNotifier",
        )
...
class TestHeartbeat:
    ...
    async def test_heartbeat_task_cancelled_after_completion(self, mocker: MockerFixture) -> None:
        ...
        mocker.patch("cloud_agents.workflow.executor.temporal.activities.activity.heartbeat")
...
class TestHTTPTranscriptCollection:
    ...
    async def test_http_transcript_connection_error_returns_empty(
        ...
        mock_logger = mocker.patch("cloud_agents.workflow.executor.temporal.activities.logger")
...
class TestTranscriptPersistence:
    ...
    async def test_full_transcript_saved_to_store(self, mocker: MockerFixture) -> None:
        ...
        mocker.patch(
            "cloud_agents.workflow.executor.temporal.activities._collect_transcript",
