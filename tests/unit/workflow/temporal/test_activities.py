...
class TestSandboxAuth:
    """Tests for per-spawn bearer token authentication (issue #10, Phase 1).

    When SANDBOX_AUTH_ENABLED=true:
    - A one-time token is generated and passed as SANDBOX_AUTH_TOKEN env var
    - The Authorization: Bearer header is sent in the httpx POST to /v1/agent/run
    - wait_ready() is NOT called with auth (health endpoint stays unauthenticated)

    When SANDBOX_AUTH_ENABLED is unset or false (default):
    ...
    def _mock_spawner(self, mocker: MockerFixture) -> Any:
        """Create a mock spawner that returns a ready endpoint."""
        mock_spawner = mocker.AsyncMock()
        mock_spawner.spawn.return_value = "http://pod-1:8080"
        mock_spawner.wait_ready.return_value = True
        return mock_spawner
    ...
    def _mock_http_success(self, mocker: MockerFixture) -> Any:
        ...
        Returns the mock client instance for assertion inspection.
        """
        mock_response = mocker.MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
    ...
    @pytest.mark.asyncio
    async def test_token_injected_as_env_var_when_enabled(
        self, mocker: MockerFixture
    ) -> None:
        """SANDBOX_AUTH_ENABLED=true injects SANDBOX_AUTH_TOKEN into sandbox env vars."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": "true"})
        mock_spawner = self._mock_spawner(mocker)
        self._mock_http_success(mocker)

        await run_sandbox_step(self._make_input(), spawner=mock_spawner)

        spawn_call = mock_spawner.spawn.call_args
        env_vars = spawn_call[1].get("env", {})
        assert "SANDBOX_AUTH_TOKEN" in env_vars
        token = env_vars["SANDBOX_AUTH_TOKEN"]
        # token_urlsafe(32) produces ~43 chars of URL-safe base64
        assert len(token) >= 32
        assert isinstance(token, str)

    @pytest.mark.asyncio
    async def test_auth_header_sent_in_http_post_when_enabled(
        self, mocker: MockerFixture
    ) -> None:
        """SANDBOX_AUTH_ENABLED=true sends Authorization: Bearer header in POST."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": "true"})
        mock_spawner = self._mock_spawner(mocker)
        mock_client = self._mock_http_success(mocker)
        ...
        # The token in the header should match the one in env_vars
        spawn_call = mock_spawner.spawn.call_args
        env_token = spawn_call[1]["env"]["SANDBOX_AUTH_TOKEN"]
        assert headers["Authorization"] == f"Bearer {env_token}"

    @pytest.mark.asyncio
    async def test_no_token_when_auth_disabled(
        self, mocker: MockerFixture
    ) -> None:
        """Default (no SANDBOX_AUTH_ENABLED) does not inject token or header."""
        mocker.patch.dict("os.environ", {}, clear=False)
        ...
        # No token in env vars
        spawn_call = mock_spawner.spawn.call_args
        env_vars = spawn_call[1].get("env", {})
        assert "SANDBOX_AUTH_TOKEN" not in env_vars

        # No Authorization header in POST
        post_call = mock_client.post.call_args
        headers = post_call[1].get("headers", {})
        assert "Authorization" not in headers

    @pytest.mark.asyncio
    async def test_no_token_when_auth_explicitly_false(
        self, mocker: MockerFixture
    ) -> None:
        """SANDBOX_AUTH_ENABLED=false does not inject token or header."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": "false"})
        mock_spawner = self._mock_spawner(mocker)
        mock_client = self._mock_http_success(mocker)
    ...
    @pytest.mark.asyncio
    async def test_unique_token_per_spawn(
        self, mocker: MockerFixture
    ) -> None:
        """Each spawn generates a different token (not reused)."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": "true"})
        ...
        for _ in range(3):
            ...
            await run_sandbox_step(self._make_input(), spawner=mock_spawner)
            spawn_call = mock_spawner.spawn.call_args
            tokens.append(spawn_call[1]["env"]["SANDBOX_AUTH_TOKEN"])

        # All tokens should be unique
        assert len(set(tokens)) == 3

    @pytest.mark.asyncio
    async def test_wait_ready_not_called_with_auth(
        self, mocker: MockerFixture
    ) -> None:
        """wait_ready() is called without auth headers (health stays unauthenticated)."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": "true"})
        mock_spawner = self._mock_spawner(mocker)
        self._mock_http_success(mocker)
        ...
        # wait_ready should be called with just endpoint and health_path
        # (no auth_header argument)
        wait_call = mock_spawner.wait_ready.call_args
        assert wait_call == mocker.call("http://pod-1:8080", health_path="/health")

    @pytest.mark.asyncio
    async def test_auth_enabled_case_insensitive(
        self, mocker: MockerFixture
    ) -> None:
        """SANDBOX_AUTH_ENABLED=True (capital T) also enables auth."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": "True"})
        mock_spawner = self._mock_spawner(mocker)
        mock_client = self._mock_http_success(mocker)
    ...
    @pytest.mark.asyncio
    async def test_auth_enabled_with_value_1(
        self, mocker: MockerFixture
    ) -> None:
        """SANDBOX_AUTH_ENABLED=1 also enables auth."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": "1"})
        mock_spawner = self._mock_spawner(mocker)
        mock_client = self._mock_http_success(mocker)
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("value", ["yes", "on", "enabled", ""])
    async def test_junk_env_values_treated_as_disabled(
        self, mocker: MockerFixture, value: str
    ) -> None:
        """Only 'true' and '1' enable auth; other truthy-looking values do not."""
        mocker.patch.dict("os.environ", {"SANDBOX_AUTH_ENABLED": value})
        mock_spawner = self._mock_spawner(mocker)
        mock_client = self._mock_http_success(mocker)
