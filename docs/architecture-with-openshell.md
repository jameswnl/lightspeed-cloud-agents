...
**Known limitation (issue #263):** `step_runner.py` forwards `OTEL_EXPORTER_OTLP_ENDPOINT`/`OTEL_EXPORTER_OTLP_PROTOCOL` into the sandbox's `env_vars` when set on the runner, so the sandbox's own OTEL-aware process knows where to export spans. This env forwarding does **not** add an egress rule for the collector host -- none of the three rules above cover it. Unless the collector happens to be reachable via an already-allowed host (or the sandbox's network policy is otherwise permissive), Landlock will block the export traffic even though the env var is set. Additionally, the sandbox image's own tracer currently always uses the gRPC exporter regardless of `OTEL_EXPORTER_OTLP_PROTOCOL`, so setting that variable to `http/protobuf` has no effect until the sandbox image itself (a separate repo, `lightspeed-agentic-sandbox`) adds protocol-aware exporter selection.
