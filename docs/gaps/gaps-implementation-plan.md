...
2. Add skills_image support (init container equivalent via OpenShell exec) — compute driver now auto-detected via the gateway's own `GetGatewayInfo()` RPC instead of a caller-supplied `driver` param that silently defaulted to "podman" regardless of the real gateway (issue: none filed separately, see PR that also fixed the `_build_baseline_filesystem_policy()` Landlock write-grant gap for `/app/skills`) ✅ for routing + Landlock policy on both real podman- and kubernetes-driver gateways; **still blocked end-to-end on kubernetes driver** by a separate, deeper UID-ownership mismatch (`/app/skills` created as uid 1001 by the pod's init process, but the sandbox's own exec'd process runs as uid 1000 per the reference image's `USER` directive) inside OpenShell's own supervisor/agent-sandbox-controller internals — needs its own tracked issue and investigation
