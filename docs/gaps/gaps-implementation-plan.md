...
2. Add skills_image support (init container equivalent via OpenShell exec) — compute driver now auto-detected via the gateway's own `GetGatewayInfo()` RPC instead of a caller-supplied `driver` param that silently defaulted to "podman" regardless of the real gateway ([PR #200](https://github.com/jameswnl/lightspeed-cloud-agents/pull/200), also fixed the `_build_baseline_filesystem_policy()` Landlock write-grant gap for `/app/skills`) ✅ for routing + Landlock policy on both real podman- and kubernetes-driver gateways; **still blocked end-to-end on kubernetes driver** by a separate, deeper UID-ownership mismatch (`/app/skills` created as uid 1001 by the pod's init process, but the sandbox's own exec'd process runs as uid 1000 per the reference image's `USER` directive) inside OpenShell's own supervisor/agent-sandbox-controller internals — tracked in [issue #201](https://github.com/jameswnl/lightspeed-cloud-agents/issues/201). Command injection via request-supplied `skills_paths` (interpolated unescaped into a shell string across all three spawners, `docs/CODE-REVIEW.md` finding #3) fixed with argv-form `cp` everywhere and the OpenShell Podman SDK/CLI fallback removed entirely (crane-only, fail loud) ([issue #202](https://github.com/jameswnl/lightspeed-cloud-agents/issues/202)) ✅; adding `crane` to the runner Containerfile so the OpenShell k8s path actually works remains open under [issue #106](https://github.com/jameswnl/lightspeed-cloud-agents/issues/106)
