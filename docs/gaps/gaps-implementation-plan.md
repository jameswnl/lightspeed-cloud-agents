...
**What was built**:
1. `_create_provider()` gained a `provider_type: str = "cloud-agents"` parameter, forwarded to `datamodel_pb2.Provider(type=provider_type, ...)` instead of the hardcoded literal. Default preserved for backward compat (non-inference credential injection).
...
3. `_do_spawn()`'s credential-provider block now reads `LIGHTSPEED_PROVIDER`/`LIGHTSPEED_MODEL` from the `env` dict it already receives (always set by `step_runner.run_step()`, the only caller that passes `credential_secret_name`) as the real `provider_type`/`model_id`, passes the real type to `_create_provider()`, and calls `_set_inference_route()` right after -- but only when both a real provider type (not the "cloud-agents" fallback) and a model are actually present, so callers/tests that only want credential injection see unchanged behavior. A `_set_inference_route()` failure fails the whole spawn (same exception path as provider-creation failure) -- fail fast and loud rather than let the sandbox discover it 30+ seconds later via repeated `GetInferenceBundle` polling.
...
5. 10 new tests: `TestProviderResponseMessages` gained `test_create_provider_uses_real_provider_type` / `test_create_provider_defaults_to_cloud_agents_type`; new `TestSetInferenceRoute` (2 tests: correct gRPC fields, gateway-rejection propagation); new `TestDoSpawnInferenceRouteWiring` (4 tests: route set when both provider+model present, skipped when model missing, skipped when provider type defaults to "cloud-agents", spawn fails when route setup fails). Updated `test_spawn_with_credential_does_not_expose_real_value`'s assertion to include the now-forwarded `provider_type`.
...
**Secondary wrinkle, not fully explained**: while manually reproducing this live, `GetInferenceBundle` succeeded for a couple of calls right after a successful `SetInferenceRoute`, then started failing with `NOT_FOUND` again a few seconds later with no new `SetInferenceRoute` call and nothing that should have invalidated the route. Not root-caused -- possibly related to per-request ephemeral provider/route churn once multiple spawns are involved in quick succession, or incidental to testing out-of-band via a standalone script rather than the real spawn path. Worth watching for once this fix is exercised via a real end-to-end spawn cycle.

**Known, unverified limitation**: giving each spawn its own named route (item 6) only prevents the *write* side from clobbering another spawn's route. Whether `GetInferenceBundle`'s caller (OpenShell's own closed-source supervisor process running inside each sandbox) correctly picks out *its own* named route among however many now exist in the workspace -- rather than e.g. always resolving the most-recently-set one, regardless of name -- is not verified here; that resolution logic isn't visible from this repo. Needs live-gateway testing with genuinely concurrent spawns to confirm end-to-end.
